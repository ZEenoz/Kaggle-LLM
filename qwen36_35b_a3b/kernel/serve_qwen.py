#!/usr/bin/env python3
"""
Serve Qwen3.6-35B-A3B on Kaggle 2x Tesla T4 GPUs with llama.cpp and Cloudflare Named / Quick Tunnel.
    ...
    if CFG.get("ntfy_topic"):
        try:
            ...
            req = urllib.request.Request(
                ...
                headers={"Title": f"Kaggle Qwen3.6: {phase}", "Tags": "robot"}
            )
...
def get_or_download_model(repo_id: str, filename: str, direct_url: str) -> Path:
    ...
    if Path("/kaggle/input").exists():
        ...
        for m in mounted:
            if filename.lower() in m.name.lower() or "35b" in m.name.lower():
    ...
    # 3. Download model
    notify("model_download", f"Downloading {filename} (~20 GB) to scratch space...")
    try:
...
def wait_for_server_ready(port: int, api_key: str, timeout_sec: int = 360):
    notify("loading_vram", "Loading model weights into 2x Tesla T4 VRAM (Context: 196k)...")
    start_t = time.time()
