#!/usr/bin/env python3
"""
kaggle-gpu-lab launcher — Serve Qwen3.6-35B-A3B on free Kaggle 2x Tesla T4 GPUs from your local terminal.
...
STATE_FILE = Path.home() / ".kaggle-qwen36-lab.json"
...
PHASE_MESSAGES = {
    ...
    "model_download": "Downloading Qwen3.6-35B-A3B GGUF (~20 GB) to scratch space...",
...
def render_live_banner(extra):
    ...
    model = extra.get("model", "qwen3.6-35B-A3B")
    ...
    print("\n" + "=" * 74)
    print("🚀 QWEN3.6-35B-A3B SERVER ONLINE (2x TESLA T4)")
    print("=" * 74)
  ...
  -d '{{"model": "{model}", "messages": [{{"role": "user", "content": "สวัสดีครับ Qwen 3.6"}}]}}'""")
...
def cmd_serve(args):
    ...
    slug = args.slug
    topic = "kqwen36-" + uuid.uuid4().hex[:16]
...
def main():
    parser = argparse.ArgumentParser(description="Kaggle GPU Lab: Serve Qwen 3.6 35B-A3B on free 2x Tesla T4 GPUs")
    subparsers = parser.add_subparsers(dest="command", required=True)

    # serve command
    p_serve = subparsers.add_parser("serve", help="Deploy Qwen3.6-35B to Kaggle 2xT4 and stream output")
    p_serve.add_argument("--slug", default="qwen36-35b-2x-t4-server", help="Kaggle kernel slug name")
    ...
    p_serve.add_argument("--model-name", default="qwen3.6-35B-A3B", help="Model alias name for API")
