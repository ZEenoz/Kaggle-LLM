#!/usr/bin/env python3
"""v2 throughput & context benchmark for Nex-N2.5-mini (Q4_K_M) on 2x Tesla T4.
...
USER_AGENT = "nex25-benchmark-v2/1.0"
...
def default_workdir() -> Path:
    return Path("/kaggle/tmp/nex25_bench" if Path("/kaggle").exists() else "/tmp/nex25_bench")
...
def write_outputs(out_dir: Path, results: dict, synthesis: dict,
    ...
    md = ["# Nex-N2.5-mini Q4_K_M -- v2 Throughput & Context Benchmark", ""]
    ...
    if rec and "suggested_launcher_config" in rec:
        ...
        md += [
            "", "### Copy-Paste for Kaggle Launcher / serve_nex.py",
            "```python",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="v2 throughput & context benchmark (KV matrix, context bisection, "
                    "batching/concurrency) for Nex-N2.5-mini Q4_K_M on 2xT4.")
