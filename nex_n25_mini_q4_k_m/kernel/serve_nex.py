#!/usr/bin/env python3
"""
Serve Nex-N2.5-mini (Q4_K_M) on Kaggle 2x Tesla T4 GPUs with llama.cpp and Cloudflare Named / Quick Tunnel.
...
DEFAULTS = {
    "model_repo": "abenzerps/Nex-N2.5-mini-GGUF",
    "model_file": "Nex-N2.5-Mini-Q4_K_M.gguf",
    "model_url": "https://huggingface.co/abenzerps/Nex-N2.5-mini-GGUF/resolve/main/Nex-N2.5-mini-Q4_K_M.gguf",
    ...
    if CFG.get("ntfy_topic"):
        try:
            ...
            req = urllib.request.Request(
                ...
                headers={"Title": f"Kaggle Nex-N2.5-mini: {phase}", "Tags": "robot"}
            )
...
def get_or_download_model(repo_id: str, filename: str, direct_url: str) -> Path:
    # 1. Check mounted Kaggle dataset (0-second mount)
    ...
    # 3. Download model
    notify("model_download", f"Downloading {filename} (~21.2 GB) to scratch space...")
    try:
...
def wait_for_server_ready(port: int, api_key: str, timeout_sec: int = 360):
    notify("loading_vram", "Loading model weights into 2x Tesla T4 VRAM (Context: 128k)...")
    start_t = time.time()
