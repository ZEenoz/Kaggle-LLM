#!/usr/bin/env python3
"""
Serve Qwen3.8-35B-A3B on Kaggle 2x Tesla T4 GPUs with llama.cpp and Cloudflare Named / Quick Tunnel.
...
DEFAULTS = {
    "model_repo": "empero-ai/Qwen3.8-35B-A3B-Distill-GGUF",
    "model_file": "Qwen3.8-35B-A3B-Q4_K_M.gguf",
    "model_url": "https://huggingface.co/empero-ai/Qwen3.8-35B-A3B-Distill-GGUF/resolve/main/Qwen3.8-35B-A3B-Q4_K_M.gguf",
    "model_name": "qwen3.8-35B-A3B",
    "context_size": 196608,
    "batch_size": 1024,
    "ubatch_size": 512,
    "kv_cache_k": "q8_0",
    ...
    if CFG.get("ntfy_topic"):
        try:
            ...
            req = urllib.request.Request(
                ...
                headers={"Title": f"Kaggle Qwen3.8: {phase}", "Tags": "robot"}
            )
...
def get_or_download_model(repo_id: str, filename: str, direct_url: str) -> Path:
    ...
    if Path("/kaggle/input").exists():
        ...
        for m in mounted:
            if filename.lower() in m.name.lower() or "qwen3.8" in m.name.lower() or "qwen38" in m.name.lower():
    ...
    # 3. Download model
    notify("model_download", f"Downloading {filename} (~20.2 GB) to scratch space...")
    try:
        ...
        if shutil.which("aria2c"):
            subprocess.run([
                ...
                "-d", str(MODEL_DIR), "-o", filename, url
            ], check=True)
        else:
            subprocess.run([
